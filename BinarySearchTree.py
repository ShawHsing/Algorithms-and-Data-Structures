class BinarySearchTree:
    ...
    def _put(self, key, val, current_node):
        if key < current_note.key:
            if current_note.has_left_child():
                self._put(key, val, current_node.left_child)
            else:
                current_node.left_childe = TreeNode(key, val, parent = current_node)
        else:
    ...
    def splice_out(self):
        if self.is_leaf():
            if self.is_left_child():
                self.parent.lef_child = None
            else:
                self.parent.right_child = None
        elif self.has_any_children():
            if self.has_left_child():
                if self.is_left_chid():
